...
class CriminalRecordRepositoryMock():
    ...
    def __init__(self):
        ...
        self.criminals = [
            Criminal(name="Lonnie Machin", nickname="Anarky", description="Anarky uses his abilities to bring down any corporation or authoritative entity that he feels hurts and oppresses people. While he can throw a punch as well as anyone, he relies primarily on his thorough understanding of modern technology to defeat his foes along with the very essence of anarchy itself—surprise.",
                     gender=GENDER.MALE, region=REGION.NEW_GOTHAM, blood_type=BLOOD_TYPE.UNDEFINED, age=16, weight=75, height=1.70),
        ...
        self.crimes = [
            Crime(crime_id=str(crime_generated_id), crime_type=CRIME_TYPE.ARSON,
                  criminal=self.criminals[0], date=1679951134400, description="Crime committed violently against innocent people", region=REGION.AMUSEMENT_MILE, seriousness=SERIOUSNESS.HIGH),
            Crime(crime_id=str(crime_generated_id), crime_type=CRIME_TYPE.THEFT,
                  criminal=self.criminals[0], date=1679951629699, description="Armed robbery against the elderly", region=REGION.NEW_GOTHAM, seriousness=SERIOUSNESS.MEDIUM),
            Crime(crime_id=str(crime_generated_id), crime_type=CRIME_TYPE.KIDNAPPING,
                  criminal=self.criminals[1], date=1679951748647, description="Sold marijuana and methamphetamine", region=REGION.FOUNDER_ISLAND, seriousness=SERIOUSNESS.HIGH),
            Crime(crime_id=str(crime_generated_id), crime_type=CRIME_TYPE.DRUG_DEALING,
                  criminal=self.criminals[1], date=1679951748647, description="Kidnapping of an orphaned child", region=REGION.FOUNDER_ISLAND, seriousness=SERIOUSNESS.LOW),
            Crime(crime_id=str(crime_generated_id), crime_type=CRIME_TYPE.MURDER,
                  criminal=self.criminals[2], date=1262304000000, description="Assassination of the president's family", region=REGION.NEW_GOTHAM, seriousness=SERIOUSNESS.HIGH),
            Crime(crime_id=str(crime_generated_id), crime_type=CRIME_TYPE.ARSON,
                  criminal=self.criminals[2], date=946684800000, description="Setting fire in 10 houses in one night, all the victims where from a organization", region=REGION.OLD_GOTHAM, seriousness=SERIOUSNESS.HIGH),
            Crime(crime_id=str(crime_generated_id), crime_type=CRIME_TYPE.DRUG_DEALING,
                  criminal=self.criminals[3], date=1679951748647, description="Seeling drugs to 16 years old in alleys close to schools, resident areas and hospitals", region=REGION.NEW_GOTHAM, seriousness=SERIOUSNESS.MEDIUM),
            Crime(crime_id=str(crime_generated_id), crime_type=CRIME_TYPE.MURDER,
                  criminal=self.criminals[3], date=946684800250, description="Assanation of many doctors", region=REGION.INDUSTRIAL_DISTRICT, seriousness=SERIOUSNESS.MEDIUM),

        ]
        self.criminal_records = [
            CriminalRecord(criminal=self.criminals[0],
                            crime_list=[self.crimes[0], self.crimes[1]],
                            criminal_record_id=str(criminal_record_generated_id),
                            danger_score=3,
                            is_arrested=False),
            CriminalRecord(criminal=self.criminals[1],
                            crime_list=[self.crimes[2], self.crimes[3]],
                            criminal_record_id=str(criminal_record_generated_id),
                            danger_score=2,
                            is_arrested=True,
            ...
            CriminalRecord(criminal_record_id=str(criminal_record_generated_id),
                            ...
                            danger_score=4,
                            is_arrested=False,
                            crime_list=[self.crimes[4],self.crimes[5]]),
            CriminalRecord(criminal=self.criminals[3],
                            crime_list=[self.crimes[6], self.crimes[7]],
                            criminal_record_id=str(criminal_record_generated_id),
                            danger_score=3,
                            is_arrested=False),
        ]
    ...
    def create_criminal_record(self, criminal_record: NewCriminalRecord) -> CriminalRecord:
        self.criminals_record.append(new_user)
        self.user_counter += 1
        return new_user
