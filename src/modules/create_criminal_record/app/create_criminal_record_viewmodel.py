...
class CreateCriminalRecordViewmodel:
    """Viewmodel responsible for translate the criminal record with a message into a json when the criminal record is created"""
    criminal_record: CriminalRecordViewmodel

    def __init__(self, criminal_record: CriminalRecord):
        """Create Criminal Record Viewmodel constructor"""
        self.criminal_record = CriminalRecordViewmodel(criminal_record)

    def to_dict(self):
        """
            Function responsible to translate the criminal record with a message into a json when the criminal record is created
        """
        return {
