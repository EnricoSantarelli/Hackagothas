...
class CreateCriminalRecordController:
    """Controller of the route Create Criminal Record"""
    ...
    def __call__(self, request: HttpRequest) -> HttpResponse:
        try:
            ...
            if not request.data.get('new_criminal_record_id'):
           ...
           # validation if the new_is_arrested is bool. It raises a entity error if returns false
            ...
            new_criminal_record = self.createCriminalRecordUsecase(new_criminal_owner=new_criminal_owner, new_criminal_record_id=request.data.get(
                "new_criminal_record_id"), new_danger_score=new_danger_score, new_is_arrested=new_is_arrested, new_prison=new_prison)

            viewmodel = CreateCriminalRecordViewmodel(new_criminal_record)
